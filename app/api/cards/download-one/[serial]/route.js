...
function getConfig(resolution) {
  const key = String(resolution || 'hd').toLowerCase()
...
export async function GET(req, { params }) {
  try {
    ...
    if (!card.public_id) {
      return new NextResponse('Public ID kartu belum tersedia.', {
        status: 400,
      })
    }
    ...
    const resolution =
      (searchParams.get('resolution') || 'hd').toLowerCase()

    const allowed = ['standard', 'hd', 'ultra', 'print']
    ...
    ctx.lineWidth = config.width * 0.004
    ...
    console.error('DOWNLOAD ONE ERROR:', error)

    return new NextResponse('Gagal membuat PNG', {
      status: 500,
    })
  }
}